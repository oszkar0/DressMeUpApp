package com.dressmeupapp.retrofit.urls;

public class Urls {
    public static final String API_URL = "https://192.168.1.100:8443";
    public static final String REGISTER = "/auth/register";
    public static final String LOGIN = "/auth/login";
    public static final String REFRESH = "/auth/refresh";
    public static final String RADIUS = "/posts/inRadius";
    public static final String CURRENT_USER = "/users/currentUser";
    public static final String DELETE_POST = "/posts";
    public static final String CREATE_POST = "/posts";
    public static final String UPDATE_USER = "/users";
    public static final String GET_POST_COMMENTS = "posts/rates";
    public static final String RATE_EXISTS = "/posts/rates/rate-exists";
    public static final String CREATE_RATE = "/posts/rates";
    public static final String DELETE_RATE = "/posts/rates";
    public static final String GET_POST = "/posts";
    public static final String GEO_API_KEY = "6af58c41d3a94226ab05085ddce90cf2";
    public static final String GEO_BASE_URL = "https://api.geoapify.com/";
}
